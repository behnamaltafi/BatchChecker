...
{
    public class SubsidySubscriberService : BackgroundService
    {
        ...
        public SubsidySubscriberService(
            ILogger logger,
        ...
        private async Task BackgroundProcessing(CancellationToken stoppingToken)
        {
            try
            {
                _logger.Information($"SubsidySubscriberService.BackgroundProcessing Start");

                var queueName = _configuration.GetSection("RabbitQueueName").Get<RabbitMqCheckerQueueName>();
                _logger.Information($"SubsidySubscriberService.BackgroundProcessing Queuenames {JsonConvert.SerializeObject(queueName)}");
                _subscriber.Subscribe(queueName.SubSidyQueueName, ProcessMessage);
                await Task.Delay(10000, stoppingToken);
                await Task.FromResult(0);

                _logger.Information($"SubsidySubscriberService.BackgroundProcessing End");
            }
            catch (Exception ex)
            {
                _logger.ErrorLog($"SubsidySubscriberService.BackgroundProcessing Exception:", ex);
            }
        }
        ...
        {
            _logger.Information($"SubsidySubscriberService.ProcessMessage Start:{message}");
            ...
            _logger.Information($"SubsidySubscriberService.ProcessMessage Result:{task.Result.ToString()}");
            return true;
            //  _logger.Information($"Subscriber Message From RabbitMQ: {JsonConvert.SerializeObject(customerResponse)}");
        }
    }
}
