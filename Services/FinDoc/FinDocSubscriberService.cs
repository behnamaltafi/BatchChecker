...
namespace BatchChecker.Services.FinDoc
{
    ...
    public class FinDocSubscriberService : BackgroundService
    {
        ...
        public FinDocSubscriberService(
            ILogger logger,
        ...
        private async Task BackgroundProcessing(CancellationToken stoppingToken)
        {
            try
            {
                _logger.Information($"FinDocSubscriberService.BackgroundProcessing Start");

                var queueName = _configuration.GetSection("RabbitQueueName").Get<RabbitMqCheckerQueueName>();
                _logger.Information($"FinDocSubscriberService.BackgroundProcessing Queuenames {JsonConvert.SerializeObject(queueName)}");

                _subscriber.Subscribe(queueName.FinDocQueueName, ProcessMessage);
                await Task.Delay(10000, stoppingToken);
                await Task.FromResult(0);

                _logger.Information($"FinDocSubscriberService.BackgroundProcessing End");
            }
            catch (Exception ex)
            {
                _logger.ErrorLog($"FinDocSubscriberService.BackgroundProcessing Exception:", ex);
            }
        }
        ...
        {
            _logger.Information($"FinDocSubscriberService.ProcessMessage Start:{message}");
            ...
            _logger.Information($"FinDocSubscriberService.ProcessMessage Result:{task.Result.ToString()}");
            return true;
        }
    }
}
