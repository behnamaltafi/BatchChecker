...
namespace BatchChecker.Services.SamaCheque
{

    public class SamaChequeSubscriberService : BackgroundService
    {
        ...
        public SamaChequeSubscriberService(
            ILogger logger,
        ...
        private async Task BackgroundProcessing(CancellationToken stoppingToken)
        {
            try
            {
                _logger.Information($"SamaChequeSubscriberService.BackgroundProcessing Start");

                var queueName = _configuration.GetSection("RabbitQueueName").Get<RabbitMqCheckerQueueName>();
                _logger.Information($"SamaChequeSubscriberService.BackgroundProcessing Queuenames {JsonConvert.SerializeObject(queueName)}");

                _subscriber.Subscribe(queueName.SamaChequeQueueName, ProcessMessage);
                await Task.Delay(10000, stoppingToken);
                await Task.FromResult(0);

                _logger.Information($"SamaChequeSubscriberService.BackgroundProcessing End");
            }
            catch (Exception ex)
            {
                _logger.ErrorLog($"SamaChequeSubscriberService.BackgroundProcessing Exception:", ex);
            }
        }
        ...
        {
            _logger.Information($"SamaChequeSubscriberService.ProcessMessage Start:{message}");
            ...
            _logger.Information($"SamaChequeSubscriberService.ProcessMessage Result:{task.Result.ToString()}");
            return true;
        }
    }
}
