...
namespace BatchChecker.Services.NocrInquiry
{
    ...
    public class NocrInquirySubscriberService : BackgroundService
    {
        ...
        public NocrInquirySubscriberService(
            ILogger logger,
        ...
        private async Task BackgroundProcessing(CancellationToken stoppingToken)
        {
            try
            {
                _logger.Information($"NocrInquirySubscriberService.BackgroundProcessing Start");

                var queueName = _configuration.GetSection("RabbitQueueName").Get<RabbitMqCheckerQueueName>();
                _logger.Information($"NocrInquirySubscriberService.BackgroundProcessing Queuenames {JsonConvert.SerializeObject(queueName)}");

                _subscriber.Subscribe(queueName.NocrInquiryQueueName, ProcessMessage);
                await Task.Delay(10000, stoppingToken);
                await Task.FromResult(0);

                _logger.Information($"NocrInquirySubscriberService.BackgroundProcessing End");
            }
            catch (Exception ex)
            {
                _logger.ErrorLog($"NocrInquirySubscriberService.BackgroundProcessing Exception:", ex);
            }
        }
        ...
        {
            _logger.Information($"NocrInquirySubscriberService.ProcessMessage Start:{message}");
            ...
            _logger.Information($"NocrInquirySubscriberService.ProcessMessage Result:{task.Result.ToString()}");
            return true;
        }
    }
}
