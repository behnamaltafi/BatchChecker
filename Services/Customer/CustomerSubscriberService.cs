...
namespace BatchChecker.Services.Customer
{
    ...
    public class CustomerSubscriberService : BackgroundService
    {
        ...
        private async Task BackgroundProcessing(CancellationToken stoppingToken)
        {
            try
            {
                _logger.Information($"CustomerSubscriberService.BackgroundProcessing Start");

                var queueName = _configuration.GetSection("RabbitQueueName").Get<RabbitMqCheckerQueueName>();
                _logger.Information($"CustomerSubscriberService.BackgroundProcessing Queuenames {JsonConvert.SerializeObject(queueName)}");

                _subscriber.Subscribe(queueName.CustomerQueueName, ProcessMessage);
                await Task.Delay(10000, stoppingToken);
                await Task.FromResult(0);

                _logger.Information($"CustomerSubscriberService.BackgroundProcessing End");
            }
            catch (Exception ex)
            {
                _logger.ErrorLog($"CustomerSubscriberService.BackgroundProcessing Exception:", ex);
            }
        }
        ...
        {
            _logger.Information($"CustomerSubscriberService.ProcessMessage Start:{message}");
            ...
            _logger.Information($"CustomerSubscriberService.ProcessMessage Result:{task.Result.ToString()}");
            return true;
        }
    }
}
