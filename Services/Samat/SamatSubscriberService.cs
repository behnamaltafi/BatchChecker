...
namespace BatchChecker.Services.Samat
{
    ...
    public class SamatSubscriberService : BackgroundService
    {
        ...
        private async Task BackgroundProcessing(CancellationToken stoppingToken)
        {
            try
            {
                _logger.Information($"SamatSubscriberService.BackgroundProcessing Start");

                var queueName = _configuration.GetSection("RabbitQueueName").Get<RabbitMqCheckerQueueName>();
                _logger.Information($"SamatSubscriberService.BackgroundProcessing Queuenames {JsonConvert.SerializeObject(queueName)}");

                _subscriber.Subscribe(queueName.SamatQueueName, ProcessMessage);
                await Task.Delay(10000, stoppingToken);
                await Task.FromResult(0);

                _logger.Information($"SamatSubscriberService.BackgroundProcessing End");
            }
            catch (Exception ex)
            {
                _logger.ErrorLog($"SamatSubscriberService.BackgroundProcessing Exception:", ex);
            }
        }
        ...
        {
            _logger.Information($"SamatSubscriberService.ProcessMessage Start:{message}");
            ...
            _logger.Information($"SamatSubscriberService.ProcessMessage Result:{task.Result.ToString()}");
            return true;
        }
    }
}
