...
namespace BatchChecker.Services.Paya
{
    ...
    public class PayaSubscriberService : BackgroundService
    {
        ...
        private async Task BackgroundProcessing(CancellationToken stoppingToken)
        {
            try
            {
                _logger.Information($"PayaSubscriberService.BackgroundProcessing Start");

                var queueName = _configuration.GetSection("RabbitQueueName").Get<RabbitMqCheckerQueueName>();
                _logger.Information($"PayaSubscriberService.BackgroundProcessing Queuenames {JsonConvert.SerializeObject(queueName)}");

                _subscriber.Subscribe(queueName.PayaQueueName, ProcessMessage);
                await Task.Delay(10000, stoppingToken);
                await Task.FromResult(0);

                _logger.Information($"PayaSubscriberService.BackgroundProcessing End");
            }
            catch (Exception ex)
            {
                _logger.ErrorLog($"PayaSubscriberService.BackgroundProcessing Exception:", ex);
            }
        }
        ...
        {
            _logger.Information($"PayaSubscriberService.ProcessMessage Start:{message}");
            ...
            _logger.Information($"PayaSubscriberService.ProcessMessage Result:{task.Result.ToString()}");
            return true;
        }
    }
}
