...
{
    public class StaffSubscriberService : BackgroundService
    {
        ...
        private async Task BackgroundProcessing(CancellationToken stoppingToken)
        {
            try
            {
                _logger.Information($"StaffSubscriberService.BackgroundProcessing Start");

                var queueName = _configuration.GetSection("RabbitQueueName").Get<RabbitMqCheckerQueueName>();
                _logger.Information($"StaffSubscriberService.BackgroundProcessing Queuenames {JsonConvert.SerializeObject(queueName)}");
                _subscriber.Subscribe(queueName.StaffQueueName, ProcessMessage);
                await Task.Delay(10000, stoppingToken);
                await Task.FromResult(0);

                _logger.Information($"StaffSubscriberService.BackgroundProcessing End");
            }
            catch (Exception ex)
            {
                _logger.ErrorLog($"StaffSubscriberService.BackgroundProcessing Exception:", ex);
            }
        }
        ...
        {
            _logger.Information($"StaffSubscriberService.ProcessMessage Start:{message}");
            ...
            _logger.Information($"StaffSubscriberService.ProcessMessage Result:{task.Result.ToString()}");
            return true;
