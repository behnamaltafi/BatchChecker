...
namespace BatchChecker.Services.Signature
{
    ...
    public class SignatureSubscriberService : BackgroundService
    {
        ...
        public SignatureSubscriberService(
            ILogger logger,
        ...
        private async Task BackgroundProcessing(CancellationToken stoppingToken)
        {
            try
            {
                _logger.Information($"SignatureSubscriberService.BackgroundProcessing Start");

                var queueName = _configuration.GetSection("RabbitQueueName").Get<RabbitMqCheckerQueueName>();
                _logger.Information($"SignatureSubscriberService.BackgroundProcessing Queuenames {JsonConvert.SerializeObject(queueName)}");

                _subscriber.Subscribe(queueName.SignatureQueueName, ProcessMessage);
                await Task.Delay(10000, stoppingToken);
                await Task.FromResult(0);

                _logger.Information($"SignatureSubscriberService.BackgroundProcessing End");
            }
            catch (Exception ex)
            {
                _logger.ErrorLog($"SignatureSubscriberService.BackgroundProcessing Exception:", ex);
            }
        }
        ...
        {
            _logger.Information($"SignatureSubscriberService.ProcessMessage Start:{message}");
            ...
            _logger.Information($"SignatureSubscriberService.ProcessMessage Result:{task.Result.ToString()}");
            return true;
        }
    }
}
